...
A **second, independent agent** — `DeepResearchAgent` (`app/agents/deep_agent.py`, on `app.state.deep_agent`) — is built the same way on top of the same checkpointer. It wraps `deepagents.create_deep_agent` and runs autonomously: built-in `write_todos` planning, virtual-filesystem working memory, **no HITL**. Both agents draw their tools from `app/agents/tools.py` (`default_tools`); the deep agent omits the bespoke `write_plan`/`update_plan` tools (its `write_todos` supersedes them). Each session is **bound** to one agent via the `ChatSession.agent_type` column (`"academic"|"deep"`), chosen in the UI before the first message and immutable after — `chat`/`resume`/`history`/`plan` all dispatch on the session's `agent_type`. Do **not** edit `AcademicAgent` when changing the deep agent (or vice versa); shared behavior goes through `app/agents/tools.py` or `app/agents/base.py`.
